...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "helloworldgwt" because\n';
  ...
  $wnd.__gwt_activeModules['helloworldgwt'] = {
    'moduleName' : 'helloworldgwt',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/helloworldgwt.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['helloworldgwt'].superdevmode = true;

  var expectedSuffix = '/helloworldgwt.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + helloworldgwt + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:helloworldgwt';
  var devModeSessionKey = '__gwtDevModeSession:helloworldgwt';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/helloworldgwt/helloworldgwt.nocache.js';
  ...
  $wnd.__gwt_activeModules['helloworldgwt'].canRedirect = true;
  $wnd.__gwt_activeModules['helloworldgwt'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING helloworldgwt';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/helloworldgwt?user.agent=' + ua + '&_callback=' + callback;
